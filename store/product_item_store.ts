...
export const mutations: MutationTree<ProductItemState> = {
  ...
  FETCH_PRODUCT_ITEMS_BY_CATEGORY(state, { paginationData, category }) {
    state.categoryPaginationData[category] =
      state.categoryPaginationData[category] || {};
    let content: Array<any> =
      state.categoryPaginationData[category].content || [];
  ...
  FETCH_SINGLE_PRODUCT_ITEM(state, { productItem }) {
    let index = state.paginationData.content.findIndex(
      (el: any) => el.id == productItem.id
    );
...
export const actions: ActionTree<ProductItemState, RootState> = {
  ...
  fetchProductItemsByCategory({ commit }, { page, category }) {
    ...
    return new Promise((resolve, reject) => {
      this.$axios
        .$get(`/product-items/category/${category}?page=${qpage}`)
        .then(paginationData => {
          commit("FETCH_PRODUCT_ITEMS_BY_CATEGORY", {
            paginationData,
            category
          });
          resolve(true);
